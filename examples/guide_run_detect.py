...
def parse_args(args):
  ...
  parser = argparse.ArgumentParser(
    description="Example of running Detect via Koshee Protect REST API",
    add_help=True
  )
  ...
  parser.add_argument('--cf', '--config', action='store',
    dest='config', help='the detect config to run',
...
################################################################
# Step 2: run detect with the provided config option
################################################################
...
if response.status_code == 200:
  ...
  print(f"\nRunning detect on {config_name}")

  detect_response = requests.get(f"{options.url}/processes/run_detect",
