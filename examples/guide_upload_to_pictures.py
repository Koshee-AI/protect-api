...
def parse_args(args):
  ...
  parser = argparse.ArgumentParser(
    description="Example of uploading an image to the user's Pictures directory via Koshee Protect REST API",
    add_help=True
  )
  ...
  parser.add_argument('-d', '--target-dir', action='store',
    dest='target_dir', help='a directory to place the image in within Pictures, if needed')
...
##################################################################
# Step 2: upload the image to Pictures
##################################################################
...
if response.status_code == 200:
  ...
  print(f"\nUploading image to Pictures")

  detect_response = requests.post(f"{options.url}/pictures/upload",
